...
> 日期：2026-04-27  
> 版本：信号增强版 (v2，终版)  
> **注意**：本文档为 `hands_decoupled` 算法的最终文档，**取代**旧版 `hands_decoupled_algorithm_report.md`。该旧版仅为 v1 设计草案，不再反映当前实现。

---

## 零、5 曲基准评测排名

基于 5 首测试曲目的综合评分，`hands_decoupled` 在四种压缩模式中排名如下：

```
排名  模式                      综合评分
---   ---                       --------
#1    adaptive_octave_fold     最高
#2    attention_weighted       次高
#3    hands_decoupled          0.5279
#4    svsep_mpdr               最低
```

> `score_aware_theory` 已在 v3 评测中淘汰，不再纳入排名。  
> 该排名反映的是 v2（最终版）`hands_decoupled` 的实测表现。`hands_decoupled` 通过左右手解耦、三重交叉验证与和声功能优先级简化，在 5 曲均值上达到 0.5279，位列四种推荐模式第三名。
