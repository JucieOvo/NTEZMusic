...
### 4.1 标准模式（adaptive_octave_fold，默认，推荐）

和声统一八度折叠 + ref_pitch 指数平滑。参数最少、速度最快、适用面最广。
...
### 4.2 进阶模式（attention_weighted，复杂曲目推荐）

滑动窗口交叉注意力 + velocity 加权 + 精排。双层趋势平滑，适合大曲目和复杂和弦。
...
### 4.3 简化模式（hands_decoupled，适中曲目推荐）

左右手解耦 + 三重验证 + 和声功能简化。适合中等复杂度曲目。
...
**Q: 和弦听起来很混浊？**
A: 降低 `--max-chord-notes` 到 3 或 4，或切换到 `attention_weighted` 模式。
